...
const FeatureCard = ({
  ...
  return (
    <div className={cn(`p-4 sm:p-8 relative overflow-hidden bg-card`, className)}>
      {children}
    </div>
  );
};
...
const FeatureTitle = ({ children }: { children?: React.ReactNode }) => {
  return (
    <p className=" max-w-5xl mx-auto text-left tracking-tight text-foreground font-semibold text-xl md:text-2xl md:leading-snug">
      {children}
    </p>
  );
};
...
const FeatureDescription = ({ children }: { children?: React.ReactNode }) => {
  ...
  return (
    <p
      className={cn(
        "text-sm md:text-base text-left max-w-5xl mx-0 md:text-sm my-2",
        "text-muted-foreground text-left font-normal",
      )}
    >
      {renderContent()}
    </p>
...
export const SkeletonTwo = () => {
  ...
  const rotationsFirst = [3.2, -7.1, 5.9, 9.1, -0.7];
  const rotationsSecond = [-3.5, -0.2, -3.7, 1.5, 0.5];
      ...
      <div className="flex flex-row -ml-20">
        {images.map((image, idx) => (
          <motion.div
            ...
            style={{
              rotate: rotationsFirst[idx % rotationsFirst.length],
            }}
      ...
      <div className="flex flex-row">
        {images.map((image, idx) => (
          <motion.div
            ...
            style={{
              rotate: rotationsSecond[idx % rotationsSecond.length],
            }}
            variants={imageVariants}
            whileHover="whileHover"
            whileTap="whileTap"
